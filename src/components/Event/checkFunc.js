...
export const checkFunc = (sportCategory, dataTypeItem, currentPlayItem, prevPlayItem, team1Id, team2Id, matchTeamId, PREV_NHL_DS2, PREV_NHL_DS5, OffensivePlays_NFL) => {
    ...
    if (dataTypeItem.no === 'NFL-DS3-2-4') {
        if (currentPlayItem.text === undefined || currentPlayItem.end === undefined || currentPlayItem.end.team === undefined) {
            ...
            var startDowns = [1, 2, 3, 4];
            if (currentPlayItem.text.toLowerCase().includes('no play') || currentPlayItem.scoringPlay != false || startDowns.indexOf(parseInt(currentPlayItem.start.down)) == -1 || currentPlayItem.end.team.id != team1Id || currentPlayItem.start.yardsToEndzone - currentPlayItem.end.yardsToEndzone < currentPlayItem.start.distance || currentPlayItem.start.yardsToEndzone - currentPlayItem.end.yardsToEndzone >= 10) {
                status = true;
            }
        }
    }
    ...
    if (dataTypeItem.no === 'NFL-DS13-2') {
        if (currentPlayItem.text === undefined || currentPlayItem.end === undefined || currentPlayItem.end.team === undefined) {
            ...
            if (currentPlayItem.text.toLowerCase().includes('no play') || currentPlayItem.text.toLowerCase().includes('sacked') || currentPlayItem.text.toLowerCase().includes('kneels') || currentPlayItem.text.toLowerCase().includes('enforced between') || startDowns.indexOf(parseInt(currentPlayItem.start.down)) == -1 || currentPlayItem.scoringPlay != false || currentPlayItem.end.team.id != team2Id || (currentPlayItem.statYardage >= 0 && currentPlayItem.end.yardLine <= currentPlayItem.start.yardLine)) {
                status = true;
            }
        }
    }
