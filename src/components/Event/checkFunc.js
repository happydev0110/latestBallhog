...
export const checkFunc = (sportCategory, dataTypeItem, currentPlayItem, prevPlayItem, nextPlayItem, team1Id, team2Id, matchTeamId, PREV_NHL_DS2, PREV_NHL_DS5, OffensivePlays_NFL) => {
    ...
    if (dataTypeItem.no === 'NFL-DS13-2') {
        if (currentPlayItem.text === undefined || currentPlayItem.end === undefined || currentPlayItem.end.team === undefined) {
            ...
            if (currentPlayItem.text.toLowerCase().includes('no play') || currentPlayItem.text.toLowerCase().includes('sacked') || currentPlayItem.text.toLowerCase().includes('kneels') || currentPlayItem.text.toLowerCase().includes('enforced between') || currentPlayItem.text.toLowerCase().includes('PENALTY') || currentPlayItem.text.toLowerCase().includes('timeout') || startDowns.indexOf(parseInt(currentPlayItem.start.down)) == -1 || currentPlayItem.scoringPlay != false || currentPlayItem.end.team.id != team2Id || (currentPlayItem.statYardage >= 0) {
                status = true;
            }
        }
    }
